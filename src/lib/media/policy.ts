...
export function publicationApproved(a:MediaApproval) {return a.rightsStatus!=='unverified'&&a.publicationScope==='production'&&a.reviewStatus==='approved'&&Boolean(a.approvedBy?.trim()&&a.evidence?.trim()&&a.approvedAt&&!Number.isNaN(Date.parse(a.approvedAt)));}
