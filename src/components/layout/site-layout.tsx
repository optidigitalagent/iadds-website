...
import { Container, BrandWordmark, Button, Arrow } from '@/components/ui/primitives';
...
export function Footer({locale}:{locale:Locale}) {
  ...
  const essential=getNavigation(locale).filter(item=>['/services','/ai-systems','/about','/contact'].some(p=>item.href===localizedPath(locale,p)));
  return <footer className={styles.footer} data-site-footer><Container><div className={styles.footerMain}>
    <div className={styles.footerBrand}><Link prefetch={false} href={localizedPath(locale)} aria-label={brand.productLabel}><BrandWordmark /></Link></div>
    <div className={styles.footerAction}><Button href={consultationHref(locale)} location="footer">{ui.book}</Button></div>
    <nav aria-label={ui.footerNav} className={styles.footerNav}>{[...essential,{label:locale==='uk'?'Вартість':'Pricing',href:localizedPath(locale,'/pricing')}].map(item=><Link prefetch={false} href={item.href} key={item.href} data-navigation>{item.label}<Arrow diagonal /></Link>)}</nav>
    ...
    <div className={styles.footerLanguage}><LanguageSwitcher ariaLabel={`${ui.language} — ${ui.footerNav}`} /></div>
