...
export function ServiceCard({service,locale,uniform=false,priority=false}:{service:Service;locale:Locale;uniform?:boolean;priority?:boolean}) {
 ...
 return <Link prefetch={false} href={localizedPath(locale,'/services/'+service.slug)} className={styles.card+' '+(service.accent==='acid'?styles.acid:'')+' '+(uniform?styles.uniform:styles['span'+service.cardSpan])} data-reveal data-format-card={examples.length>0||undefined} data-service-card={service.slug}>
 <div className={styles.cardTop}><span>{service.professionalLabel}</span><span className={styles.cardArrow}><Arrow diagonal /></span></div>{examples.length?<div className={styles.cardMedia+' '+(examples.length===2?styles.splitPreview:'')}>{examples.map((media,index)=><FormatMedia key={media.id} media={media} card priority={priority&&index===0} labels={copy} />)}</div>:<MediaFrame media={service.cardMedia} labels={labels} hero={priority} className={styles.cardMedia} caption={false} />}<div className={styles.cardContent}>{examples.length>0&&<span className={styles.formatLabel}>{copy.label}</span>}{status&&(status!=='client'||service.cardMedia.publicationApproved)&&<span className={styles.illustrative}>{status==='client'?labels.clientWork:labels.conceptProject}</span>}<h3>{service.title}</h3><p>{service.shortPromise}</p></div></Link>;
}
