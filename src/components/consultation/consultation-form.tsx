...
import {emptyInput,fieldOrder,validateConsultation,type ConsultationInput,type FieldName,type FormErrors} from '@/lib/validation/consultation';
...
function FormField({name,label,optional,hint,error,children}:{name:string;label:string;optional?:string;hint?:string;error?:string;children:ReactNode}) {return <div className={styles.field+' '+(['companyUrl','selectedService','message','communicationLanguage'].includes(name)?styles.fullWidth:'')}><label htmlFor={'field-'+name}>{label} {optional?<span className={styles.optional}>({optional})</span>:<span aria-hidden="true">*</span>}</label>{hint&&<p id={'hint-'+name} className={styles.hint}>{hint}</p>}{children}<p id={'error-'+name} className={styles.fieldError}>{error||'\u00a0'}</p></div>;}
...
export function ConsultationForm({copy,locale,sourcePage,services,selectedService,collaborationModel,support}:Props) {
 ...
 function update(name:FieldName,value:string|boolean) {setValues(previous=>({...previous,[name]:value}));setErrors(previous=>({...previous,[name]:undefined}));setServerMessage('');}
 ...
 async function submit(event:FormEvent<HTMLFormElement>) {
  ...
  const body=JSON.stringify({...validation.data,companyFax:honeypot.current?.value||''});
  if(!attempt.current||attempt.current.body!==body) attempt.current={key:crypto.randomUUID(),body};
  try {
   const response=await fetch('/api/consultation',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':attempt.current.key},body,signal:AbortSignal.timeout(15000)});
 ...
 const inputs=[{name:'fullName',autoComplete:'name',maxLength:100},{name:'company',autoComplete:'organization',maxLength:120},{name:'role',autoComplete:'organization-title',maxLength:100,optional:true},{name:'email',autoComplete:'email',maxLength:254,type:'email'},{name:'contactMethod',autoComplete:'tel',maxLength:80,optional:true},{name:'companyUrl',autoComplete:'url',maxLength:300,type:'url'}] as const;
 ...
 {inputs.map(input=><FormField key={input.name} name={input.name} label={copy.fields[input.name]} optional={'optional' in input?copy.optional:undefined} hint={input.name==='companyUrl'?copy.hints.companyUrl:undefined} error={errors[input.name]}><input {...attributes(input.name,input.name==='companyUrl')} type={'type' in input?input.type:'text'} autoComplete={input.autoComplete} maxLength={input.maxLength} value={values[input.name]} required={!('optional' in input)} onChange={e=>update(input.name,e.target.value)} /></FormField>)}
 ...
 <FormField name="message" label={copy.fields.message} hint={copy.hints.message} error={errors.message}><textarea {...attributes('message',true)} rows={4} minLength={10} maxLength={1500} value={values.message} required onChange={e=>update('message',e.target.value)} /></FormField>
