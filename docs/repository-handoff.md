...
Keep the existing Sites source remote and owner-only audience. Push the validated commit, verify `HEAD` equals `refs/heads/main`, inspect native Sites build/publication status and the deployed source SHA, then run authenticated smoke checks against the live origin. Provider run IDs and any unavailable logs must be reported honestly. The CI script is usable by native Sites build; a local pass alone is not remote CI evidence.
