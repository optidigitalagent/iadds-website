...
| Artifact | Output | Audience | Unverified media |
|---|---|---|---|
| Local review | `.next-review` | Loopback `127.0.0.1:3100` | 30 selected assets served from private `.data/iadds-media` |
| Production / Sites | `.next`, then `dist` | Site's authorized audience | Excluded from selectors, assets, static output and archive |
...
Card videos have no eager `src` or native `poster` request. Their accessible responsive picture is the visible poster; the native player receives its source and poster only after a permitted interaction in the viewport. Cards are muted, inline, looping, reset outside the viewport and coordinated globally. Reduced motion, Save-Data and touch default to posters. Detail videos require a play action, expose native controls, start muted, preserve native ratio, and retain a poster on failure.
