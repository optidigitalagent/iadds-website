...
async function walk(dir) { const items=[]; for(const d of await fs.readdir(dir,{withFileTypes:true})) { if(['node_modules','.next','.git','.data','test-results','playwright-report'].includes(d.name)) continue; const p=path.join(dir,d.name); if(d.isDirectory()) items.push(...await walk(p)); else items.push(p); } return items; }
...
const report={createdAt:new Date().toISOString(),sourceRoot:root,counts:{unique:41,videos:27,images:14,selected:selected.size,reserve:41-selected.size,founders:7,contactSheets:7,curatedCopies:assets.reduce((n,a)=>n+a.curatedCopies.length,0)},missingOrCorrupt:[],assets,files:fileAudit};
