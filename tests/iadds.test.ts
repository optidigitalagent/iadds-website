...
import {publicationApproved,mediaEnvironment} from '../src/lib/media/policy';
...
test('all 41 source IDs retain provenance and explicit unverified rights',()=>{
 ...
 for(const a of mediaCatalog){const p=approvals[String(a.id) as keyof typeof approvals];assert.equal(p.rightsStatus,'unverified');assert.equal(selectExample(a.id,'en','production'),null);assert.match(a.sourceSha256,/^[a-f0-9]{64}$/);}
...
test('production cannot be enabled by a local flag or partial rights approval',()=>{
 ...
 for(const change of [{rightsStatus:'unverified'},{approvedAt:null},{approvedBy:null},{evidence:null},{reviewStatus:'pending'},{publicationScope:'local'}])assert.equal(publicationApproved({...approved,...change} as MediaApproval),false);
});
