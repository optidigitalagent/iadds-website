...
for(const locale of ['uk','en'] as const) {
 ...
 test(locale+' form validation, optional fields and honest provider error preserve input',async({page})=>{
  await page.goto('/'+locale+'/consultation?service=missing&from=https%3A%2F%2Fevil.example');await expect(page.locator('#field-selectedService')).toHaveValue('');await page.getByRole('button',{name:c.consultation.submit,exact:true}).click();await expect(page.locator('#field-fullName')).toBeFocused();await expect(page.getByText(c.consultation.validation.message,{exact:true})).toBeVisible();await expect(page.locator('#field-role')).not.toHaveAttribute('required');await expect(page.locator('#field-contactMethod')).not.toHaveAttribute('required');
